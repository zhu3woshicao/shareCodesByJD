...
class MMysql {
    ...
    protected $_dbName = 'dataBase'; //数据库名
     ...
     * @return mixed
     */
    protected function _doQueryone($sql='') {
        $this->_sql = $sql;
    ...
    /**
     * 执行语句 针对 INSERT, UPDATE 以及DELETE,exec结果返回添加的id
     * @param string $sql sql指令
     * @return integer
     */
    protected function _doExecId($sql='') {
        $this->_sql = $sql;
        self::$_dbh->exec($this->_sql);
     ...
     * @return array 结果集
     */
    public function first($tbName='') {
        $sql = "select ".trim($this->_field)." from ".$tbName." ".trim($this->_where)." ".trim($this->_order)." ".trim($this->_limit);

        $this->_clear = 1;
        $this->_clear();
        return $this->_doQueryone(trim($sql));
    }
